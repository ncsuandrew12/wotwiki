...
for book_index, book_ro in ipairs(m_data) do
    ...
    if book.templates ~= nil then
        for _, template in ipairs(book.templates) do
            local key = string.upper(template)
            if key ~= string.upper(book.abbrev) then
                if m.book_map[key] ~= nil then error("Duplicate book template "..template.." for books "..m.book_map[key].title.." and "..book.title..".") end
                m.book_map[key] = book
            end
        end
    end
...
function m._chapter_nav(frame, args)
    ...
    local book = m.book_map[string.upper(book_key)]
    if book == nil then
        book_key = string.upper(string.gsub(string.gsub(args["book"], "_", " "), "&#39;", "'"))
        book = m.book_map[string.upper(book_key)]
    end
